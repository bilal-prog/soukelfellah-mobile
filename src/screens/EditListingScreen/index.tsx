...
export const EditListingScreen: FC<EditListingScreenProps> = memo(
  function EditListingScreen(props) {
    ...
    const isLoading = updateListingMutation.isPending

    // Fetch listing details for editing
    const { data: listing } = useListingDetailsQuery(listingId)
    ...
    useEffect(() => {
      if (listing) {
        setSelectedCat(listing.listingType)
        setTitle(listing.title)
        setDescription(listing.description)
        setPrice(listing.price !== undefined && listing.price !== null ? String(listing.price) : "")
        setQuantity(String(listing.quantity))
        setAddress(listing.location?.address || "")

        if (listing.location?.region && dbRegions) {
          const reg = dbRegions.find((r: any) => r.name === listing.location.region)
          if (reg) setSelectedRegion(reg)
        }

        setModel(listing.modelYear || "")
        setHours(listing.hours || "")
        setListingDirection(listing.listingDirection)
        ...
        const ptId =
          typeof listing.productTypeId === "object" && listing.productTypeId !== null
            ? (listing.productTypeId as any)._id
            : listing.productTypeId
        if (ptId && productTypes) {
          const pt = productTypes.find((p: any) => p._id === ptId)
          if (pt) setSelectedProductType(pt)
        }

        const uId =
          typeof listing.unitId === "object" && listing.unitId !== null
            ? (listing.unitId as any)._id
            : listing.unitId
        if (uId && units) {
          const u = units.find((un: any) => un._id === uId)
          if (u) setSelectedUnit(u)
        }

        if (listing.images) {
          const urls = listing.images.map((img: any) => (typeof img === "object" ? img.url : img))
          const ids = listing.images.map((img: any) => (typeof img === "object" ? img._id : img))
          setImages(urls)
          setImageIds(ids)
        }
        setStep(2)
      }
    }, [listing, categories, productTypes, units, dbRegions])

    // Hydrate province once parent region's provinces are loaded
    useEffect(() => {
      if (listing?.location?.province && dbProvinces) {
        const prov = dbProvinces.find((p: any) => p.name === listing.location.province)
        if (prov) {
        ...
        { id: listingId, params: payload },
        {
          onSuccess: () => {
            ...
            Alert.alert(
              ...
              translate("myListings:updateSuccess"),
              [
                {
          ...
          {step === 2 && (
            <View style={styles.section}>
              ...
              <View style={styles.formGroup}>
                ...
                <View style={styles.inputField}>
                  <Text
                    tx="addListing:listingDirectionLabel"
                    size="xxs"
                ...
                {selectedCat === "EQUIPMENT" && (
                  <View style={styles.inputField}>
                    <Text
                      tx="addListing:listingPurposeLabel"
                      size="xxs"
                      style={styles.selectLabel}
                    />
                    <View style={styles.segmentRow}>
                      ...
                      >
                        <Text
                          ...
                          style={[
                            styles.segmentText,
                            purpose === "SELL" && styles.segmentTextActive,
                          ]}
                        />
                      </TouchableOpacity>
                      <TouchableOpacity
                      ...
                      >
                        <Text
                          ...
                          style={[
                            styles.segmentText,
                            purpose === "RENT" && styles.segmentTextActive,
                          ]}
                        />
                      </TouchableOpacity>
                    </View>
